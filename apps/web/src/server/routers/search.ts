    ...
    .query(async ({ input }) => {
      ...
      const [byType, bySentiment, byStage] = await Promise.all([
        prisma.source.groupBy({
          by: ["sourceType"], where, _count: { _all: true },
        }),
        prisma.source.groupBy({
          by: ["sentiment"], where, _count: { _all: true },
        }),
      ...
      return {
        sourceTypes: byType.map((t) => ({ value: t.sourceType, count: t._count._all })),
        sentiments: bySentiment
          .filter((s) => s.sentiment)
          .map((s) => ({ value: s.sentiment!, count: s._count._all })),
        flowStages: byStage
          .map((s) => ({
            id: s.stageId,
