...
// Server-side client — talks to storage over the internal/private endpoint
// (e.g. http://minio:9000 inside Docker, or the real S3/R2 endpoint in prod).
let _client: S3Client | null = null;
export function s3(): S3Client {
  return (_client ??= makeClient(process.env.S3_ENDPOINT));
}

// Presign client — signs URLs the *browser* will hit, so it must use a
// browser-reachable host. In Docker that's localhost:9000 (S3_PUBLIC_ENDPOINT),
// while the app's own calls keep using S3_ENDPOINT (minio:9000). Falls back to
// S3_ENDPOINT for single-host setups where they're the same.
let _presignClient: S3Client | null = null;
function presignS3(): S3Client {
  return (_presignClient ??= makeClient(
...
export async function presignGet(key: string): Promise<string> {
  return getSignedUrl(presignS3(), new GetObjectCommand({ Bucket: BUCKET, Key: key }), {
    expiresIn: 900,
  });
}
