...
/** Embeddings provider chosen by EMBED_PROVIDER (openai | local). */
...
/** LLM provider chosen by LLM_PROVIDER (anthropic | local). Used for Pass B
 *  classification, insight extraction, and summaries. Falls back to the local
 *  stub when no key is configured so the pipeline runs offline. */
